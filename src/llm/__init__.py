...
class LLMApi(object):
    ...
    def __init__(
        ...
        if embedding_api_type == "openai":
            self.embedding_service = OpenAIService
        elif embedding_api_type == "ollama":
            self.embedding_service = OllamaService
        else:
            raise ValueError(
                f"Embedding API type {embedding_api_type} is not supported",
            )
    ...
    def _stream_smoother(
            ...
            try:
                ...
                for msg in stream:
                    ...
                    with update_lock:
                        ...
                        cur_base_speed = new_speed * weight + cur_base_speed * (
                            1 - weight
                        )
        ...
        try:
            while True:
                ...
                if (
                    finished
                    and len(buffer_content) - len(sent_content) < min_chunk_size * 5
                ):
                ...
                if (
                    cur_buf_message is None
                    or len(buffer_content) - len(sent_content) < min_chunk_size
                ):
                ...
                target_speed = (
                    cur_base_speed_norm
                    + (cur_base_speed_norm - cur_actual_speed_norm) * 0.25
                )
                ...
                new_pack_size = (
                    ...
                    if rand_max - rand_min > 1
                    else rand_min
                )

                chunk_time = (
                    chunk_time_target / non_zero(new_pack_size_target) * new_pack_size
                )
